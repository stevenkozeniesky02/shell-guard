...
const handlers = {
  ...
  // Internal vault state — encrypted in-memory, persisted to disk
  // Uses envelope encryption: master key derives per-secret keys via HKDF
  _vaultState: null,

  _initVault() {
    if (this._vaultState) return this._vaultState;

    const { createHash, createCipheriv, createDecipheriv, randomBytes, createHmac } = require("crypto");

    // Derive master key from AGENT_TOKEN + PROJECT_KEY (unique per agent)
    const masterSeed = createHash("sha512")
      .update(AGENT_TOKEN + ":" + PROJECT_KEY + ":vault:v1")
      .digest();

    // Split into encryption key (32 bytes) and HMAC key (32 bytes)
    const encKey = masterSeed.subarray(0, 32);
    const hmacKey = masterSeed.subarray(32, 64);

    // Derive per-secret key using HKDF-like construction
    ...
    // Encrypt a value with per-secret derived key + random IV
    ...
    const vaultPath = resolve(CWD, ".agentsid-vault.enc");
    let secrets = {};
    let accessLog = [];
    let rotationHistory = {};
    ...
    // Save vault to disk
    function persist() {
      const data = { secrets, accessLog: accessLog.slice(-1000), rotationHistory };
      const payload = JSON.stringify(data);
      const hmac = hmacSign(payload);
      writeFileSync(vaultPath, JSON.stringify({ data, hmac, version: 1 }), "utf-8");
    }

    this._vaultState = {
      secrets, accessLog, rotationHistory,
      encrypt, decrypt, persist, hmacSign, deriveSecretKey,
    };
  ...
  async secrets_store({ name, value, metadata }) {
    ...
    const vault = handlers._initVault();
  ...
  // ── Secrets: Scope (restrict which tools can use a secret) ──
  async secrets_scope({ name, allowedTools }) {
    ...
    vault.secrets[name].allowedTools = tools;
    vault.persist();

    return success(`Secret "${name}" scoped to tools: ${tools.join(", ")}`, toolName);
  },
  ...
  // ── Secrets: Proxy Inject (THE KEY FEATURE) ──
